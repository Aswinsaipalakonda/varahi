...
export default function OverviewPage() {
  ...
  useEffect(() => {
    ...
    if (userRole === 'owner') {
      ...
      // Supervisor: fetch investments and payouts directly to calculate metrics client-side
      Promise.all([
        ...
        .then(([investments, payouts]) => {
          ...
          setData({
            pending_verifications: pendingTxns.length,
            pending_payouts: pendingPayouts.length,
            active_investors: new Set(investments.filter((i: any) => i.status === 'active').map((i: any) => i.customer_mobile)).size,
            total_aum: investments.reduce((acc: number, curr: any) => acc + parseFloat(curr.amount || 0), 0)
          });
          setLoading(false);
        })
        .catch((err) => {
  ...
  const statCards = role === 'owner' ? [
    ...
    { 
      ...
      value: (data.pending_payouts || 0).toString(), 
      change: 'Action Required',
      labelUnder: 'DISBURSEMENT QUEUE',
      icon: Clock, 
      color: 'bg-[#1D173C]',
  ...
  return (
    <div className="space-y-8 font-sans text-slate-800">
      ...
      <div className={`grid grid-cols-1 md:grid-cols-2 ${role === 'owner' ? 'lg:grid-cols-4' : 'lg:grid-cols-3'} gap-6`}>
          ...
          const CardContent = (
            <div className={`p-6 rounded-[24px] ${card.color} text-white relative overflow-hidden transition-all duration-200 hover:-translate-y-1 shadow-lg shadow-indigo-900/5`}>
              ...
              <div className="mt-6 pt-3 border-t border-white/5 flex items-center gap-2 text-[9px] font-bold font-sans">
                <span className={card.change.startsWith('+') || card.change === 'Active' ? 'text-emerald-400' : 'text-slate-400'}>{card.change}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {role === 'owner' ? (
          ...
          /* Operational Tasks Overview for Supervisor */
          <div className="lg:col-span-2 p-6 bg-white border border-slate-200 rounded-[28px] shadow-sm flex flex-col justify-between">
            <div>
              <div className="pb-4 border-b border-slate-100 mb-6">
                <h3 className="text-base font-bold text-slate-900">Operations Tasks Overview</h3>
                <p className="text-xs text-slate-500 mt-0.5">Assigned queues for compliance and payout verifications</p>
              </div>
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                  <Link href="/dashboard/transactions" className="p-4 rounded-2xl bg-slate-50 border border-slate-100 hover:border-blue-500/40 transition-all flex flex-col justify-between">
                    <div>
                      <h4 className="text-xs font-extrabold text-slate-800 uppercase tracking-wider font-mono">Verify Deposits</h4>
                      <p className="text-[10px] text-slate-500 mt-1">Audit customer deposit receipts and verify bank credit matches.</p>
                    </div>
                    <span className="text-xs font-semibold text-blue-600 hover:underline mt-4 flex items-center gap-1">
                      Go to Queue <ArrowRight size={12} />
                    </span>
                  </Link>
                  <Link href="/dashboard/payouts" className="p-4 rounded-2xl bg-slate-50 border border-slate-100 hover:border-blue-500/40 transition-all flex flex-col justify-between">
                    <div>
                      <h4 className="text-xs font-extrabold text-slate-800 uppercase tracking-wider font-mono">Process Payouts</h4>
                      <p className="text-[10px] text-slate-500 mt-1">Confirm bank transfers and disburse monthly payouts to customers.</p>
                    </div>
                    <span className="text-xs font-semibold text-blue-600 hover:underline mt-4 flex items-center gap-1">
                      Go to Queue <ArrowRight size={12} />
                    </span>
            ...
            <div className="pt-6 border-t border-slate-100 flex gap-4 mt-6">
              <Link href="/dashboard/transactions" className="text-xs font-semibold text-blue-600 hover:underline flex items-center gap-1">
                <span>Audit Transactions</span>
                <ArrowRight size={12} />
              </Link>
              <Link href="/dashboard/payouts" className="text-xs font-semibold text-blue-600 hover:underline flex items-center gap-1">
